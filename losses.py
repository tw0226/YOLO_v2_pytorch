...
class DetectionLoss(nn.Module):
    ...
    def compute_ious(self, box1, boxes, off_x, off_y):
        new_boxes = torch.zeros(boxes.shape)

        new_boxes[0] = (boxes[0] + off_x) / 13
        new_boxes[1] = (boxes[1] + off_y) / 13
        new_boxes[2] = boxes[2]
        new_boxes[3] = boxes[3]
        # print(boxes[:, 0], new_boxes[:, 0])

        x1 = torch.max(box1[0] - box1[2]/2, new_boxes[0, :] - new_boxes[2, :]/2)
        y1 = torch.max(box1[1] - box1[3]/2, new_boxes[1, :] - new_boxes[3, :]/2)
        x2 = torch.min(box1[0] + box1[2]/2, new_boxes[0, :] + new_boxes[2, :]/2)
        y2 = torch.min(box1[1] + box1[3]/2, new_boxes[1, :] + new_boxes[3, :]/2)
    ...
    def forward(self, detection_result, gt):
        ...
        for batch in range(batch_size):
            ...
            for one_obj in range(0, len(gt_one), 5):
                ...
                #box1 대한 IOU 계산
                ious = self.compute_ious(one_obj_box, anchor_boxes, int(x * grid_size), int(y * grid_size))
                ...
                gt_grid[batch, index*25:index*25+5, int(y * grid_size), int(x * grid_size)] = \
                    torch.tensor([y*grid_size - int(y * grid_size), x * grid_size - int(x * grid_size), w, h, 1])
                ...
                weight_grid[batch, index * 25:index * 25 + 5, int(y * grid_size), int(x * grid_size)] = torch.tensor([5, 5, 5, 5, 1])
                weight_grid[batch, index * 25 + 5:index * 25 + 25, int(y * grid_size), int(x * grid_size)] = torch.ones(20)
                #print('index',index)
                # if(int(x * grid_size)==9):
                #     print(detection_result[0, 25 * index:25 * index+25,  int(y * grid_size),  int(x * grid_size)])
                #     print(gt_grid[0, 25 * index:25 * index+25,int(y * grid_size),  int(x * grid_size)])
